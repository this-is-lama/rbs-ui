...
На странице бронирований должна показываться текущий заказ, то есть это будет как корзина.
Например я на схеме выбрал стол, нажал забронировать, и теперь сам ресторан и стол будут храниться в localstorage.
Точно также и с блюдами, я нажимаю добавить и он будет в корзине
то есть страницу бронирования надо переделать, там же должна показываться текущая стоимость

Далее нужно переделать карточку блюда
Я хочу чтобы вес блюда был в темно-серой плашке в левом нижнем углу
А кнопка + - с ценой должны быть в правом нижнем углу 

Также убери надпись "Бронирование сейчас добавляется в localStorage как корзина бронирований. На этом этапе серверная отправка финального заказа не выполняется."

И поменяй местами схему зала и меню, сначала должно идти меню, а потом схема зала
